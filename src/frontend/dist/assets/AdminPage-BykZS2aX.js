...
function AdminPage() {
  ...
  const normalizeScheme = (raw) => {
    const name = raw.name || "Unnamed Scheme";
    const slug = raw.slug || name.toLowerCase().replace(/[^a-z0-9\s-]/g, "").replace(/\s+/g, "-").replace(/-+/g, "-").trim() || `scheme-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
    return {
      id: raw.id || `imported-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
      name,
      state: raw.state || "India",
      category: raw.category || "General",
      benefit: raw.benefit || "",
      benefit_amount_numeric: typeof raw.benefit_amount_numeric === "number" ? raw.benefit_amount_numeric : 0,
      eligibility: raw.eligibility || "",
      documents: Array.isArray(raw.documents) ? raw.documents : typeof raw.documents === "string" ? raw.documents.split("|").map((d) => d.trim()).filter(Boolean) : [],
      apply_link: raw.apply_link || raw.applyLink || "",
  ...
  const handleImportJson = () => {
    try {
      ...
      setImportJson("");
      setShowImport(false);
      ue.success(`${normalized.length} schemes import ho gaye!`);
    } catch (err) {
      ue.error(
  ...
  const handleFileUpload = async (e) => {
    ...
    if (file.name.endsWith(".json")) {
      ...
      try {
        ...
        const normalized = parsed.map(normalizeScheme);
        setSchemes([...schemes, ...normalized]);
        ue.success(`${normalized.length} schemes import ho gaye JSON se!`);
      } catch {
